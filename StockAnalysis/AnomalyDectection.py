...
#ticker_end_date = dt.date(2021,9,17)
...
dia_df = af.get_ticker_jobs(refresh_index = False,refresh_data=False, index='DOW', njobs=4, start_date=ticker_start_date)
...
sp_500_df = af.get_ticker_jobs(refresh_index = False,refresh_data=False, index='SP500', njobs=2, start_date=ticker_start_date)
...
nasdaq_df = af.get_ticker_jobs(refresh_index = False,refresh_data=False, index='NASDAQ', njobs=8, start_date=ticker_start_date)
...
etf_df = af.get_ticker_jobs(refresh_index=False,refresh_data=False,index='ETF',njobs=1,start_date=ticker_start_date)
...
gld_df = af.get_index_data(ticker = 'GLD',start_date = ticker_start_date,outfile=r'c:\users\cosmi\onedrive\desktop\gld.csv', refreshFileOutput=True)
sptl_df = af.get_index_data(ticker = 'SPTL',start_date = ticker_start_date,outfile=r'c:\users\cosmi\onedrive\desktop\sptl.csv', refreshFileOutput=True)

list_of_index_values = [sp_500_df,nasdaq_df,dia_df,etf_df]
...
"""
portfolio_file_df = af.read_csv_bulk(input_file = r'c:\users\cosmi\onedrive\desktop\portfolio.csv', file_size = 1000000000,chunk_count = 100000)
portfolio_file_df = portfolio_file_df[['ticker', 'open_position']]
#all_tickers_df = all_tickers_df.loc[(all_tickers_df['ticker']=='SOFI') | (all_tickers_df['ticker']=='GM')]
stock_age_df = pd.DataFrame(data=all_tickers_df.groupby(by=['ticker'])['close'].count())
stock_age_df.reset_index(inplace=True)
stock_age_df.rename(columns={'close':'record_count'},inplace=True)
all_tickers_df = all_tickers_df.merge(right=stock_age_df,how='inner',on='ticker')
all_tickers_df = all_tickers_df.merge(right=portfolio_file_df, how='left',on='ticker')
all_tickers_df = all_tickers_df.loc[(all_tickers_df['record_count']>= 240) | (all_tickers_df['open_position'] == 1)]
...
all_tickers_df.reset_index(drop=True,inplace=True)
price_target = [all_tickers_df['twentythreeRetracement'][x] if all_tickers_df['twentythreeRetracementDiff'][x] > 0 else all_tickers_df['thirtyeightRetracement'][x] if all_tickers_df['thirtyeightRetracementDiff'][x] > 0 else all_tickers_df['sixtytwoRetracement'][x] for x in all_tickers_df.index ]
all_tickers_df['priceTarget'] = price_target
all_tickers_df.drop(labels=['naturalLog', 'pctChange'],axis=1,inplace=True)
