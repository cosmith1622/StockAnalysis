...
def get_index_data(ticker: str,start_date: str,outfile: str=None, index_as_date: bool=False,refreshFileOutput: bool=False) ->pd.DataFrame:
    ...
    if path.exists(outfile) and refreshFileOutput==False:
        ...
        index_table['pctChange'] = index_table['close'] / index_table['previous_close']
        index_table['naturalLog'] = np.log(index_table['pctChange'])
        rolling_std = index_table['naturalLog'].rolling(20).std()
        index_table['indexRollingSTD'] = rolling_std
        index_table.reset_index(inplace=True)
        index_table.rename(columns={'index':'date'},inplace=True)
        index_table.drop(labels=['previous_close', 'pctChange', 'naturalLog'], axis=1, inplace=True)
...
def getPortfolio(portfolio_file:str, prices_file:str, index_file:str,ticker_start_date:str, ticker_end_date:str):
    ...
    model_df = all_stocks.loc[all_stocks['num_days']==30]
    #model_df = model_df.loc[(model_df['ticker']=='GM') | (model_df['ticker']=='CSX') | (model_df['ticker']=='FCX')]
    model_df = model_df[['ticker', 'score', 'slope']]
    list_of_stocks = all_stocks.loc[(all_stocks['date']==ticker_end_date) & (all_stocks['close'] <= 80) & (all_stocks['volume'].astype('int64') >= 1000000)]
    #list_of_stocks = list_of_stocks.loc[(list_of_stocks['ticker']=='GM') | (list_of_stocks['ticker']=='CSX') | (list_of_stocks['ticker']=='FCX')]
    all_stocks = all_stocks.merge(right=list_of_stocks,how='inner',left_on=['ticker'], right_on=['ticker'])
    all_stocks.rename(columns={"close_x" : "close", "date_x": "date", "ticker_x" : "ticker", "priceTarget_x" : "priceTarget", "onestandarddeviationmove_x" : "onestandarddeviationmove", "rollingSTD_x" : "rollingSTD"},inplace=True)
    all_stocks = all_stocks[['close', 'date', 'ticker','priceTarget', 'onestandarddeviationmove', 'rollingSTD']]
    all_stocks = all_stocks.merge(right=model_df,how='inner',on='ticker')
    companies_info = get_companies_info(tickers=list_of_stocks['ticker'])
    ...
    all_stocks = all_stocks[['close','date','ticker', 'priceTarget', 'open_position','quantity', 'industry', 'sector', 'onestandarddeviationmove', 'rollingSTD', 'score', 'slope']]
    all_stocks.reset_index(inplace=True)
    open_position = (all_stocks['open_position'][x] if all_stocks['open_position'][x] == 1 else 0 for x in all_stocks.index)
    all_stocks['open_position'] = [x for x in open_position]
    ...
    row_group = analysis_corr.groupby('ticker')['level_1'].max()
    row_group = row_group.reset_index()
    analysis_corr = analysis_corr.merge(right=row_group, how='inner', left_on=['ticker','level_1'], right_on=['ticker','level_1'])   
    ...
    analysis_corr = analysis_corr[['ticker','priceTarget','exit_price','open_position','quantity','sector','industry','close', 'index_pct_change', 'onestandarddeviationmove', 'rollingSTD', 'score', 'slope']]
    analysis_corr.rename(columns={"priceTarget": "entry_price", "index_pct_change": "market_corr", "score":"r2"}, inplace = True)
    analysis_corr['Pct_To_Entry_Price'] = (analysis_corr['close'] - analysis_corr['entry_price'] ) / analysis_corr['entry_price']
    analysis_corr = getStockBeta(index_df=spy_index_df, df = analysis_corr, ticker_end_date=ticker_end_date)
...
def getStockBeta(index_df:pd.DataFrame, df:pd.DataFrame, ticker_end_date:str) -> pd.DataFrame: 

    index_df = index_df.loc[index_df['date']==ticker_end_date]
    index_df = index_df[['date', 'indexRollingSTD']]
    df['beta'] = df['market_corr'] * (df['rollingSTD']/index_df['indexRollingSTD']._values[0])
    return df
